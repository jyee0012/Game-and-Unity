...
public class TankBaseMovement : MonoBehaviour
{
    ...
    void ChangeChildren()
    {
        if (childTurret == null) childTurret = GetComponentInChildren<TankTurretScript>();
        childTurret.useController = useController;
        childTurret.playerNum = playerNum;
    }
    ...
    void UpdateText()
    {
        if (enemyText != null)
        {
            enemyText.text = enemyName + ": " + enemyScore;
        }
        if (healthText != null)
        {
            healthText.text = "P" + playerNum + " Health: " + health;
        }
    }
}
