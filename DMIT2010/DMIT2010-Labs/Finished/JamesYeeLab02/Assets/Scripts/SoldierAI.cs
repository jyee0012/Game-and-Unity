...
public class SoldierAI : MonoBehaviour
{
    ...
    bool shot = false, foundNoise = false, newRight, newLeft;
    float shotTimer;
    int shotCount = 0, MoveAroundWall = 0;
    ...
    void Detect()
    {
        ...
        {
            if ((hitObjectCenter.transform.tag != "Survivor" && (hitObjectCenter.transform.tag != "Zombie" || hitObjectCenter.transform.tag != "Soldier")) && !(hitObjectCenter.transform.tag == "Survivor" && myTarget != null))
            {
                if (myTarget == null)
                {
                    ...
                    // If something is to the right then face the direction of the normal for the object and then turn 90 degrees counterclockwise
                    ...
                    // If something is to the left then face the direction of the normal for the object and then turn -90 degrees counterclockwise
                    else if (!rightDetect && leftDetect)
                    {
                        Turn(hitObjectCenter.normal, direction.Right);
                        newRight = rightDetect;
                        MoveAroundWall--;
                    }
        ...
        else if (myState == AIState.wander)
        {
    ...
    void AI()
    {
        switch (myState)
        {
            ...
            case AIState.wander:
                ...
                {
                    speed = defaultSpeed;
                    zombies = Physics.OverlapSphere(transform.position, 1.0f);
            ...
            case AIState.attack:
                if (myTarget != null)
                {
                    ...
                    if (Vector3.Distance(transform.position, myTarget.position) < 6.0f && myTarget.tag == "Zombie")
                    {
                        CallOut();
    ...
    public void FollowTarget(Transform target, bool IsZom)
    {
        if (target != null)
        {
            if (Physics.SphereCast(transform.position, 3.0f, transform.forward, out searchHit))
            {
                if (searchHit.transform.tag == "Soldier" && !myTarget)
                {
                    myTarget = target;
                }
                else
    ...
    void MakeNoise()
    {
        Collider[] noise = Physics.OverlapSphere(transform.position, 10.0f);
        foreach (Collider entity in noise)
        {
            ...
            else if (entity.tag == "Soldier" && entity.transform.position != transform.position)
            {
                entity.GetComponent<SoldierAI>().FollowTarget(gameObject.transform, false);
            }
    ...
    void CallOut()
    {
        Collider[] call = Physics.OverlapSphere(transform.position, 5.0f);
        foreach (Collider entity in call)
        {
            if (entity.tag == "Soldier" && entity.transform.position != transform.position)
            {
                entity.GetComponent<SoldierAI>().FollowTarget(gameObject.transform, false);
            }
    ...
    void OnDestroy()
    {
        Collider[] zomEat = Physics.OverlapSphere(transform.position, 2.0f);
        foreach (Collider zom in zomEat)
        {
            if (zom.tag == "Zombie")
            {
                zom.GetComponent<ZombieAIOpt>().Eat();
            }
        }
    }
    ...
    public void FindNoise(Transform target)
    {
        if (target != null)
        {
            if (Physics.SphereCast(transform.position, 3.0f, transform.forward, out searchHit))
