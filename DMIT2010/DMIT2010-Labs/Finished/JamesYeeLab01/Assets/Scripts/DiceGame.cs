        ...
        switch (ai)
        {
            ...
            case AIState.Check:
                ...
                else
                {
                    ...
                    if ((threeKind > -1 || twoPair > -1) && (!pCombos[player][3].selected || !pCombos[player][2].selected || !pCombos[player][1].selected || !pCombos[player][0].selected))
                    {
                    ...
                    #region Loop & checking max number in array
                    int checktime1 = 0, checktime2 = 0, checktime3 = 0;
                    for (int checktime = 0; checktime < 2; checktime++)
                    {
                        for (int i = checktime + 4; i < 6; i++)
                        {
                            for (int c = 0; c < i; c++)
                            {
                                if (diceTotals[c] > 0)
                    ...
                    #endregion
                    #region Keep if you are within the array
                    if (!pCombos[player][5].selected || !pCombos[player][4].selected && numTimesRolled < 2)
                    {
                        #region Check
                        // Checktime1 Checks 1-4, next is 2-5, then 3-6
                        if (checktime1 >= 3 || checktime2 >=3 || checktime3 >=3)
                        {
                            int straightStart = 10;
                            if (checktime1 >= 3)
                            {
                                straightStart = 0;
                            }
                            else if (checktime2 >=3)
                            {
                                straightStart = 1;
                            }
                            else if (checktime3 >= 3)
                            {
                                straightStart = 2;
                            }
                            for (int i = 0; i < 5; i++)
